...
async function main() {
  const encoding: Encoding = await createEncoding(exampleName, 'BOOM x5');

  const input = await createHttpInput(configProvider.getHttpInputHost());
  const inputFilePath = configProvider.getHttpInputFilePath();

  const output = await createGCSOutput(
      configProvider.getGCSOutputBucketName(),
      configProvider.getGCSOutputAccessKey(),
      configProvider.getGCSOutputSecretKey()
  );

  const h264VideoConfiguration = await createBaseH264VideoConfig();
  const aacAudioConfiguration = await createPCMAudioConfig(256000);
 ...
 */
function createGCSOutput(bucketName: string, accessKey: string, secretKey: string): Promise<GcsOutput> {
  const s3Output = new GcsOutput({
    bucketName: bucketName,
    accessKey: accessKey,
    secretKey: secretKey
  });

  return bitmovinApi.encoding.outputs.gcs.create(s3Output);
}
 ...
 */
function createPCMAudioConfig(bitrate: number): Promise<PcmAudioConfiguration> {
  const config = new PcmAudioConfiguration({
    name: `AAC ${bitrate / 1000} kbit/s`,
    bitrate,
    channelLayout: PcmChannelLayout.MONO,
    sampleFormat: PcmSampleFormat.S24LE
  });

  return bitmovinApi.encoding.configurations.audio.pcm.create(config);
}
...
function buildAbsolutePath(relativePath: string): string {
  return join(configProvider.getGCSOutputBasePath(), exampleName, relativePath);
}
